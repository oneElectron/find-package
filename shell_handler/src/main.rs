...
use find_core::{Repo, parse_filters, find_installed_package_managers};
...
fn main() {
    ...
    for query in args.queries { // ? Maybe we should rethink the format of the output
        let r: Vec<&SearchResult<'_>> = results.iter().filter(|r| { r.query.unwrap() == query && (1 << r.repo as u64) & filter_package_managers != 0 }).collect();
            if !r.is_empty() {
            println!(
